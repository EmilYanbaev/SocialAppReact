...
class LeftSiteBar extends React.Component {
    ...
    render() {
        let itemsJSX = this.state.items.map(
            item => <ItemLink item={item} disable={!this.props.isLogin} key={item.id} />
        )

        return (
            <div className={style.sitebar} style={this.props.isView ? { "display": "flex" } : {}} >
                {itemsJSX}
            </div>
        )
